...
const ExaminationAreaComputersIndex = () => {
  ...
  const onFinishAcceptForm = values => {
    (async () => {
      const response = await acceptOnlineApplicationsApi(
        selectedElement?.id,
        values?.message,
        {}
      );
  ...
  const onFinishRejectForm = values => {
    (async () => {
      const response = await rejectOnlineApplicationsApi(
        selectedElement?.id,
        values?.message,
        {}
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (index, row) => (
        <>
          {row?.organization?.name_uz} | {row?.group}
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (index, row) => (
        <div className={"d-flex"}>
          <Button onClick={() => selectElement(row, "reject")}>
            <i className={"fa fa-times text-danger"}></i>
          </Button>
          <Button onClick={() => selectElement(row, "accept")}>
