...
import {Table, Row, Input, Col, Select, Form , Popconfirm,message} from "antd";
                ...
                <Container fluid>
                  ...
                  <span className="mr-3" onClick={() => history.goBack()}>
                    <i className="bx bx-arrow-back"> </i>
                                    ...
                                    <h5 className="text-dark">
                                        <b>"{group?.type == 'first' ? group?.group?.name_uz : group?.name}"</b> guruhing
                                        to'lovi (<b>{group?.organization?.name_uz}</b>)
                                    </h5>
                                </div>
                                <div>
                  <span className="ml-2 mr-2">
                    <b>{selected_keys?.length}</b> ta o'quvchi
                                    ...
                                    <Popconfirm title={'To`lovni bekor qilasizmi ?'} placement={'bottom'} onConfirm={cancel_payment} >
                                    <button
                                        disabled={selected_keys?.length > 0 ? false : true}
                                        className="btn btn-warning"
                                    >
                                        {" "}
                                       <i class="fa fa-exclamation"></i> To'lovni bekor qilish
                                    </button>
                                        </Popconfirm>
                                </div>
                            </div>
                            <Row>
                                ...
                                >
                                    <Col xl={6}>
                                        ...
                                        >
                                            <Input disabled={true} className="w-100"
                                                   placeholder={'Guruh uchun qabul qilingan umumiy summani kiriting'}
                                                   type={"number"} allowClear={true}/>
                                        </Form.Item>
                                    </Col>
                                    <Col xl={6}>
                                        <Form.Item
                                        ...
                                        >
                                            ...
                                            >
                                                {
                                                    filters?.examination_area_payment_types?.map((element, index) => {
                                                        return (
                                                            <Option key={index}
