...
const StudentPaymentNotConfirmedsIndex = ({firstTabLoading}) => {
    ...
    }, [get_again, show_count, organization_id, waitWord, group_id, firstTabLoading]);
        ...
        if (selected?.id) {
            ...
            axios({
                ...
                if (res?.data?.status == 1) {
                    ...
                    {
                        res?.data?.data?.payType?.type === 'cash' ?
    ...
    return (
        <>
            <Modal title={'To`lovni tasdiqlash'}
                   ...
                   footer={[
                       <Button key={'back'} onClick={handleCancel}>Bekor qilish</Button>,
                       <Button key={'submit'} type="primary" loading={isLoading} onClick={handleOk}>Saqlash</Button>
                   ]}
            >
                <Row>
                    ...
                    <Col xl={12} className="text-center d-flex justify-content-center">
                        <b>{selected?.final_access_group?.type == 'first' ? selected?.final_access_group?.group?.name_uz : selected?.final_access_group?.name} ({selected?.organization?.name_uz})</b>
                    </Col>
                </Row>
                <Form
                    form={confirm_form}
                    ...
                    >
                        <Thead className="font-size-14">
                            <Tr>
                                ...
                                <Th data-priority="3" className={'text-center'}>Kelish sanasi va vaqti</Th>
                            ...
                            {data?.map((element, index) => {
                                return (
                                    <Tr key={index} className={'hover-tr-success'}
                                        ...
                                        <Td>
                      <span style={{cursor: 'pointer'}} onClick={() => confirm_payment(element)}>
                        {element?.student_fio}
                        </span>
                                        </Td>
                                        ...
                                        <Td className={'text-center'}>
                                            <button className={'btn btn-outline-success'}
                                                    onClick={() => confirm_payment(element)}><i
...
export default withTranslation()(StudentPaymentNotConfirmedsIndex);
