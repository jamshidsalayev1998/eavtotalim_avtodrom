...
import {Row, Col, Select, Input, Pagination, DatePicker, Tabs, Button,Table} from "antd";
...
const GeneralPaymentReportTab = () => {
    ...
    const [intervalDate, setIntervalDate] = useState({
        from_date: localStorage.getItem(pageConfig?.fromDate) ? moment(localStorage.getItem(pageConfig?.fromDate)) : null,
        to_date: localStorage.getItem(pageConfig?.toDate) ? moment(localStorage.getItem(pageConfig?.toDate)) : moment()
    });
    ...
    const columnsHead = [
        ...
        {
            title: 'UMUMIY',
            render: (index, element) => <>{summAmount(element?.payments)}</>
        },

    ];
    const columnsFoot =
        filters ? filters?.examinationAreaPaymentTypes?.map((elemType, indexType) => {
            return {
                title: elemType?.name,
                children: filters ? filters?.paymentTypes?.map((elemType2, indexType2) => {
                    return {
                        title: elemType2?.short_name,
                        render: (index, element) => <>{summAmount(element?.payments?.filter(perElem => perElem.examination_area_payment_type_id === elemType?.id)?.filter(perElemT => perElemT.payment_type_id === elemType2?.id))}</>,
                    }
                }) : []
            }
        }) : [];
    const columns = Object.assign(columnsFoot, columnsHead);
    const exportExamResultFunction = () => {
        ...
        Object.assign(params, {paymentFilters: JSON.stringify(paymentFilters)});
        exportPaymentReportFunction('dsd',params)
    };
    return (
        <>
                ...
                <Row className="d-flex ">
                    ...
                    <Col xl={8}>
                        ...
                        >
                            ...
                            {
                                filters?.users ? filters?.users?.map((element, i) => {
                                    return (
