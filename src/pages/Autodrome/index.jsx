...
const Autodrome = () => {
  ...
  const getAutodromes = async () => {
    return await axios
      .get(`${PATH_PREFIX_V2}/examination-area-organization-agreement`, {
        params: {
          token: token,
          page: selected_count,
        },
  ...
  const agreementHandle = async (id, key, data) => {
    let res = await axios.post(
      `${PATH_PREFIX_V2}/examination-area-organization-agreement/${
        key === "accept" ? "accept" : "reject"
      }/${id}`,
      data,
      {
        headers: {
          ...
          <CardBody>
                  ...
                  {autodromes?.data?.map((el, index) => {
                    return (
                      <Tr key={el?.id}>
                        ...
                        <Th className="table_body">{el?.organization?.name}</Th>
                        <Th className="table_body">{el?.agreement_number}</Th>
                        <Th className="table_body">
                          {el?.agreement_expiration_date}
                        </Th>
                        <Th className="table_body">
                          <Badge
                            color={
                        ...
                        <Th>
                          <a
                            className="link_to_print"
                            href={PATH_PREFIX_FILE + el?.agreement}
                            download
                            target="_blank"
                            rel="noreferrer"
                          >
                          ...
                          {el?.status === 0 ? (
                            <div className="func_buttons">
                              <Tooltip title={t("confirm")}>
                                <span
                                  onClick={() =>
                                    agreementHandle(el?.id, "accept", el)
                                  }
                                >
                              ...
                              <Tooltip title={t("reject")}>
                                <span
                                  onClick={() =>
                                    agreementHandle(el?.id, "reject", el)
                                  }
                                >
                                  <i className="bx bx-x font-size-18 mr-3 text-danger"></i>
                                </span>
                              </Tooltip>
                            </div>
                          ) : el?.status === 1 ? (
                            ...
                            <Tooltip title={t("confirmed_archive")}>
                              <span>
                                <i className="bx bx-archive font-size-24"></i>
                              </span>
                            </Tooltip>
                          ) : (
                            ""
            ...
            {autodromes.data?.length > 0 && (
              <div className="table_bottom_tools_custom">
                <div className="show_responsive"></div>
