...
import {Row, Col, Select, Input, Pagination, Table, Modal, Form, message, Popconfirm} from "antd";
import axios from "axios";
import {PATH_PREFIX} from "../../../Utils/AppVariables";
import styleAction from "./style.module.css"
import moment from "moment";

const CashierIndex = props => {
    ...
    useEffect(() => {
        ...
        setIsloading(true)
        axios({
            url: PATH_PREFIX + '/examination-director/cashier',
            method: 'GET',
            params: {
                token,
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (index, element) => <div className="d-flex" style={{fontSize:'20px'}}>
                <span  className={styleAction?.action_buttons} style={{color: '#fa8c16'}} onClick={()=>show_cashier(element)}><i className="bx bx-show-alt"></i></span>
                <span className={styleAction?.action_buttons}  style={{color: '#1890ff'}}  onClick={()=>edit_cashier(element)}><i className="bx bx-edit"></i></span>
                 <Popconfirm title={'Ma`lumotni o`chirasizmi'} placement="topLeft"
                            onConfirm={() => delete_cashier(element?.id)} okText={'O`chirish'}
                            cancelText={'Bekor qilish'} >
                     <span className={styleAction?.action_buttons} style={{color: '#f5222d'}}> <i className="bx bxs-trash"></i></span>
                </Popconfirm>
            </div>,
    ...
    const delete_cashier = (cashier_id) => {
        const token = localStorage.getItem('token');
        axios({
            url:PATH_PREFIX+'/examination-director/cashier/'+cashier_id,
            method:'DELETE',
            params:{
                token,
                cashier_id
    ...
    const showCashierLoginParol = (cashier_id) => {
        const token = localStorage.getItem('token');
        axios({
            url:PATH_PREFIX+'/examination-director/cashier-get-password',
            method:'GET',
            params:{
                token,
                cashier_id
            ...
            if (res?.data?.status == 1){
                setShowLoginParol(prevState => ({...prevState, username:res?.data?.data?.username , password: res?.data?.data?.password}))
            }
        })
    ...
    const onFinishEdit = (values) => {
        ...
        const token = localStorage.getItem('token');
        axios({
            url: PATH_PREFIX + '/examination-director/cashier/'+editCashier?.id,
            method: 'PATCH',
            params: {
                token,
                 ...
                 <Row>
                     ...
                     <Col xl={12}>
                      ...
                      <Col xl={12}>
                         ...
                         <button className="btn btn-light" onClick={()=>showCashierLoginParol(showedCashier?.id)}> <i className="fa fa-eye"></i> </button>
                     </Col>
                      <Col xl={12}>
                         <p>
                             Login: <b>{showLoginParol?.username ? showLoginParol?.username:'________'}</b>
                         </p>
                           <p>
                             Parol: <b>{showLoginParol?.password ? showLoginParol?.password:'________'}</b>
                         </p>
                     </Col>
                    ...
                    <Card>
                        <CardBody>
                            <div className="top-organizations">
                                <h5>Kassirlar </h5>
                                <button className="btn btn-outline-success" onClick={showAddModal}> + Qo'shish</button>
                            </div>
...
export default withTranslation()(CashierIndex);
