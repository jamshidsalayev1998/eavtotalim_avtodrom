...
const ExaminationAreaSensorsIndex = props => {
    ...
    const [eduTypeId, setEduTypeId] = useState(localStorage.getItem(pageConfig?.eduTypeId) ? localStorage.getItem(pageConfig?.eduTypeId) :'all');
    ...
    function edit_payment(element) {
        ...
        setFileEditList([{
            ...
            name: element?.image,
            status: 'done',
            url: PATH_PREFIX_FILE + element?.image
        }]);
    ...
    const columns = [
        {
            title: 'Nomi',
            render:(index,element) => <NavLink to={'/examination-area/sensors/'+element?.id}>{element?.name }</NavLink>
        },
        {
            title: 'Jarima bali',
        ...
        {
            title: 'Rasm',
            render: (index, element) => <><img style={{maxWidth: '100px'}} src={PATH_PREFIX_FILE + element?.image}
                                               alt=""/></>
        },
        {
            title: 'Ta`lim turi',
    ...
    const changeEduTypeID = (value) => {
        localStorage.setItem(pageConfig?.eduTypeId,value != 'all' ? parseInt(value) :'all');
        setEduTypeId(value != 'all' ? parseInt(value) :'all');
        setReload(!reload);
    };
    return (
        <>
            <AddExaminationAreaSensorModal dummyRequest={dummyRequest} setIsAddModalVisible={setIsAddModalVisible}
                                           isAddModalVisible={isAddModalVisible} setReload={setReload}
                                           reload={reload}/>
            <EditExaminationAreaSensorModal dummyRequest={dummyRequest} setFileEditList={setFileEditList}
                                            fileEditList={fileEditList} setIsEditModalVisible={setIsEditModalVisible}
                                                ...
                                                eduTypes?.map((element, index) => {
                                                    return (
                                                        <Option value={element?.id}
                                                                key={index}>{element?.short_name_uz}</Option>
                                                    )
                                                })
                                            }
                                        </Select>
