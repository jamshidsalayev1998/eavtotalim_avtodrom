...
const FinalAccessAdmin = props => {
        ...
        },
        {
            ...
            render: (index, element) => <div className="d-flex" style={{fontSize: '20px'}}>
                <span className={styleAction?.action_buttons} style={{color: '#fa8c16'}}
                      onClick={() => show_cashier(element)}><i className="bx bx-show-alt"></i></span>
                <span className={styleAction?.action_buttons} style={{color: '#1890ff'}}
                      onClick={() => edit_cashier(element)}><i className="bx bx-edit"></i></span>
    ...
    const delete_cashier = (cashier_id) => {
        const token = localStorage.getItem('token');
        axios({
            url: PATH_PREFIX + '/examination-director/final_access_admin/' + cashier_id,
            method: 'DELETE',
            params: {
                token,
                cashier_id
    ...
    const showCashierLoginParol = (cashier_id) => {
        const token = localStorage.getItem('token');
        axios({
            url: PATH_PREFIX + '/examination-director/final-access-admin-get-password',
            method: 'GET',
            params: {
                token,
                cashier_id
    ...
    const onFinishEdit = (values) => {
        ...
        const token = localStorage.getItem('token');
        axios({
            url: PATH_PREFIX + '/examination-director/final_access_admin/' + editCashier?.id,
            method: 'PATCH',
            params: {
                token,
            ...
            >
                <Row>
                    ...
                    <Col xl={12}>
                        ...
                        <button className="btn btn-light" onClick={() => showCashierLoginParol(showedCashier?.id)}><i
