...
import {Table, Row, Col, Tabs, message, notification, Input, Modal, Button} from "antd";
import {allowSeparatelyStudent, getExamProcessStudents} from "../../../../services/api_services/final_test_admin_api";
            ...
            >
                ...
                <Row style={{fontSize: "16px"}}>
                    ...
                    <Col xl={12} className={'text-right'}>
                        <Badge
                            color={parseInt(data?.payment_status) === 1 ? 'success' : parseInt(data?.payment_status) === 2 ? 'dark' : 'danger'}>
                            {parseInt(data?.payment_status) === 1 ? 'To`lov qilingan' : parseInt(data?.payment_status) === 2 ? 'To`lov qilinmaydi' : 'To`lov qilinmagan'}
                        </Badge>
                    </Col>
                    <Col xl={12} className={'text-left'}>Test topshirganlik holati</Col>
                    <Col xl={12} className={'text-right'}>
                        <Badge
                            color={parseInt(data?.exam_result) === 1 ? 'success' : parseInt(data?.exam_result) === 2 ? 'dark' : data?.exam_result === null ? 'warning' : 'danger'}>
                            {parseInt(data?.exam_result) === 1 ? 'Testdan o`tgan' : parseInt(data?.exam_result) === 2 ? 'Test topshirmaydi' : data?.exam_result === null ? 'Test topshirmagan' : 'Testdan yiqilgan'}
                        </Badge>
                    </Col>
                    <Col xl={12} className={'text-left'}>Testga ruhsat berish holati</Col>
                    <Col xl={12} className={'text-right'}>
                        <Badge
                            color={parseInt(data?.status) === 2 ? 'success' : 'warning'}>{parseInt(data?.status) === 2 ? 'Ruxsat berilgan' : 'Ruxsat berilmagan'}</Badge>
                    </Col>
                    <Col xl={24} className={'text-center border p-3'}>
                        <h1><b>{data?.merged_computer ? data?.merged_computer?.computer?.order : ' '}</b></h1>
                        <br/>
                        Kompyuter raqami
                    </Col>
                    <Col xl={24} className={'text-center mt-1'}>
                        {
                            parseInt(data?.status) === 1 && parseInt(data?.payment_status) === 1 ?
                                <button className={'w-100 btn btn-success'}
                                        onClick={() => allowFinalExam(data?.id)}>Testga ruxsat berish</button> : ''
