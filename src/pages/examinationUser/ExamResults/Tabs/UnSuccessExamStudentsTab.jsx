...
const UnSuccessExamStudentsTab = (props) => {
    const {Option} = Select;
    ...
    const [intervalDate, setIntervalDate] = useState({
        from_date: localStorage.getItem(window.location.pathname + '-unsuccess-filter-from_date') ? moment(localStorage.getItem(window.location.pathname + '-unsuccess-filter-from_date')) : null,
        to_date: localStorage.getItem(window.location.pathname + '-unsuccess-filter-to_date') ? moment(localStorage.getItem(window.location.pathname + '-unsuccess-filter-to_date')) : moment()
    });
    ...
    const changeIntervalDate = (value) => {
        ...
        if (value?.length > 0) {
            ...
            localStorage.setItem(window.location.pathname + '-unsuccess-filter-from_date', value[0]);
            localStorage.setItem(window.location.pathname + '-unsuccess-filter-to_date', value[1]);
        } else if (value == null) {
            setIntervalDate({
                from_date: null,
                to_date: moment()
            })
            localStorage.removeItem(window.location.pathname + '-unsuccess-filter-from_date');
            localStorage.removeItem(window.location.pathname + '-unsuccess-filter-to_date');
        }
    }
    ...
    const exportExamResultFunction = () => {
        ...
        exportExamResult('Testdan yiqilganlar', params)
    }
            ...
            <div className="crypto-buy-sell-nav mt-3">
                    ...
                    <Col xl={8}>
                        ...
                        <ReactToPrint
                            trigger={() => {
                                ...
                                return <Button>Kunlik bayonnoma chiqarish ({intervalDate?.to_date ? moment(intervalDate?.to_date).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD')})</Button>;
                            }}
                ...
                <Row className="d-flex justify-content-end mt-2">
                 ...
                 <div className={'print-box'} ref={(element) => printReportRef = element}>
                    <PrintReportByDay dataPrint={data} statusPrint={'un_success'}
...
export default UnSuccessExamStudentsTab