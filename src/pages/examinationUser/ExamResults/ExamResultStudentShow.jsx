...
const ExamResultStudentShow = props => {
  ...
  useEffect(() => {
    ...
    axios({
      url:
        PATH_PREFIX +
        "/examination-user/results-by-student/" +
        match_params?.params?.id,
      method: "GET",
      params: {
        token,
      },
            ...
            <CardBody>
              <div className="top-organizations">
                <div className="top_links_page_title">
                  ...
                  <h5 className="text-dark">Imtihon natijalari</h5>
                </div>
                <div className={"d-flex "}>
                  <button className="btn btn-light mx-1" onClick={handlePrint}>
                    Natijalarni chop qilish <i className="fa fa-print"></i>
                  </button>
                  <ReactToPrint
                    trigger={() => {
                      ...
                      return (
                        <button className="btn btn-light" onClick={handlePrint}>
                          Bayonnoma chop qilish <i className="fa fa-print"></i>
                        </button>
                      );
                    }}
                      ...
                      {data &&
                        data?.map((element, index) => {
                          return (
                            <Col
                              xl={24}
                              className="border p-3 mt-2 mb-2 hover-shadow"
                              key={index}
                            >
                              <div>
                                <Badge
                              ...
                              <div>
                                ...
                                {element?.question?.body &&
                                  Object.entries(element?.question?.body)?.map(
                                    ([key, question_b]) => {
                                      return (
                                        <>
                                          {question_b?.type == 1 ? (
                                            <span>{question_b?.value}</span>
                                          ) : question_b?.type == 2 ? (
                                            <>
                                              {" "}
                                              <br />{" "}
                              ...
                              <div className="w-100 mt-2 ">
                                <p>Javoblar:</p>
                                {Object.entries(element?.answers)?.map(
                                  ([key_answer, value_answer]) => {
                                    return (
                                      <div
                                        className="p-3 border"
                                        style={{
                                          backgroundColor:
                                            value_answer?.bg_color,
                                        }}
                                      >
                                        {Object.entries(
                                          value_answer?.body
