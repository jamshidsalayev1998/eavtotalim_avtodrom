  ...
  const columns = [
    ...
    },
    {
      ...
      render: (index, element) => (
        <div>
          {element?.final_practical_test_records
            ? element?.final_practical_test_records?.map(elementMap => {
                return (
                  <Popover
                    content={
                      <div>
                        <div>
                          Avtomobil:{" "}
                          <b>
                            {elementMap?.examination_area_car?.name} -{" "}
                            {elementMap?.examination_area_car?.number}
                          </b>
                        </div>
                        <div>
                          Jarima bali:{" "}
                          <b>
                            {elementMap?.penalty_ball
                              ? elementMap?.penalty_ball
                              : 0}
                          </b>
                        </div>
                        <div>
                          Natija:{" "}
                          <b>
                            {parseInt(elementMap?.result) === 1 ? (
                              <Badge color={"success"}>O`tgan</Badge>
                            ) : parseInt(elementMap?.result) === 0 ? (
                  ...
                  >
                    <NavLink
                      to={{
                        ...
                        state: {
                          edu_type_id:
                            elementMap?.examination_area_car?.edu_type_id,
                    ...
                    >
                      <div
                        ...
                        className={`hover-result-box border p-1 hover rounded ${
                          parseInt(elementMap?.result) === 1
                            ? `bg-success-box`
                            : parseInt(elementMap?.result) === 0
                            ? `bg-danger-box`
                      ...
                      >
                        {elementMap?.examination_area_car?.name} -{" "}
                        {elementMap?.examination_area_car?.number} (
                        {elementMap?.penalty_ball} - jarima bali){" "}
                      </div>
                    </NavLink>
                  </Popover>
                );
