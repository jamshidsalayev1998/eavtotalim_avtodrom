...
import {Row, Col, Table, Popconfirm, Form, message, Tooltip} from "antd"
import {
    clearComputerApi,
    deleteComputersApi,
    indexComputersApi,
...
import {PATH_PREFIX, PATH_PREFIX_FILE, PATH_PREFIX_FILE_WITHOUT_SLESH} from "../../../Utils/AppVariables";
...
const ExaminationAreaComputersIndex = () => {
    ...
    const columns = [
        ...
        {
            title: 'Holat',
            render: (index, element) => <> {element?.merge ? <Badge color={'warning'}>Band</Badge> :
            ...
            render: (index, element) => <>
                <div className="d-flex" style={{fontSize: '20px'}}>
                    <Tooltip title={'Kalit faylni yuklash'}>
                        <a download={element?.order+'_key.json'}
                           href={'data:text/json;charset=utf-8,' + '{"key":"'+element?.key+'"}'}
                           className={styleAction?.action_buttons}
                           style={{color: '#1890ff'}}><i
                            className="bx bx-download"/></a>
                    </Tooltip>
                    <Tooltip title={'O\'zgartirish'}>
                    <span className={styleAction?.action_buttons} onClick={() => selectForEdit(element)}
                          style={{color: '#1890ff'}}><i
                        className="bx bx-edit"/></span>
                    </Tooltip>
                    <Popconfirm title={'Ma`lumotni o`chirasizmi'} placement="topLeft"
                                okText={'O`chirish'}
                                cancelText={'Bekor qilish'} onConfirm={() => deleteComputer(element)}>
                        <Tooltip title={'O\'chirish'}>
                        <span className={styleAction?.action_buttons} style={{color: '#f5222d'}}> <i
                            className="bx bxs-trash"/></span>
                        </Tooltip>
                    </Popconfirm>
                    {element?.merge ? <Popconfirm title={'Kompyuterni bo\'shatishni hohlaysizmi'} placement="topLeft"
                                                  okText={'Bo\'shatish'}
                                                  cancelText={'Bekor qilish'} onConfirm={() => clearComputer(element)}>
                            <Tooltip title={'Kompyuterni bo\'shatish'}>
                                <span className={styleAction?.action_buttons} style={{color: '#52c41a'}}> <i
                                    class='bx bxs-brush-alt'></i> </span>
                            </Tooltip>
                        </Popconfirm> :
                        ''}
    ...
    const onFinishEditForm = (values) => {
        ...
        (async () => {
            if (selectedEditElement) {
                ...
                const response = await updateComputersApi(values, selectedEditElement?.id);
                if (parseInt(response?.status) === 1) {
                    message.success(response?.message);
                    cancelEditModal();
                    setReload(!reload)
