...
const NotAllowedSeparatelyAllowStudentsTable = ({}) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (index, element) => (
        <>
          {element?.type == "resubmit" ? "Qayta topshirish" : "Birinchi marta"}
        </>
      ),
    },
    {
      ...
      render: (index, element) => (
        <>
          {parseInt(element?.payment_status) ? (
            <Popconfirm
              title={"O`quvchiga testga ruxsat berilsinmi"}
              onConfirm={() => allow_student(element?.id)}
              okText={"Ruxsat berish"}
              cancelText={"Bekor qilish"}
            >
              <Button
                disabled={!parseInt(element?.payment_status)}
                className={
                  parseInt(element?.payment_status) === 1
                    ? "btn btn-outline-success text-success"
    ...
    if (student_id) {
      axios({
        ...
        if (res?.data?.status == 1) {
          history.push(
            `/come-examination/allow-students/separately/${res?.data?.final_access_student?.id}`
          );
